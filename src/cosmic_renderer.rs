...
impl CosmicTextRenderer {
    pub fn new() -> Self {
        info!("Initializing CosmicTextRenderer with advanced text shaping");

        // Create font system with system fonts + custom fonts
        let mut font_system = FontSystem::new();

        // Load custom fonts from fonts/ directory
        Self::load_custom_fonts(&mut font_system);
    ...
    fn load_custom_fonts(font_system: &mut FontSystem) {
        let custom_fonts = vec![
            ("fonts/anime_ace.ttf", "Anime Ace"),
            ("fonts/anime_ace_3.ttf", "Anime Ace 3"),
            ("fonts/Arial-Unicode-Regular.ttf", "Arial Unicode"),
            ("fonts/comic shanns 2.ttf", "Comic Sans"),
            ("fonts/msyh.ttc", "Microsoft YaHei"),
            ("fonts/NotoSansMonoCJK-VF.ttf.ttc", "Noto Sans Mono CJK"),
        ];

        for (path, name) in custom_fonts {
            if let Ok(font_data) = std::fs::read(path) {
                font_system.db_mut().load_font_data(font_data);
                info!("Loaded custom font: {} from {}", name, path);
            }
        }
    }
    ...
    fn render_text_internal(
        ...
        buffer.draw(&mut font_system, &mut swash_cache, cosmic_color, |px_x, px_y, _w, _h, pixel_color| {
            let img_x = x + px_x as i32;
            let img_y = y + px_y as i32;
