...
use crate::services::traits::BubbleDetector;
use crate::types::BubbleDetection;
use anyhow::Result;
...
impl DetectionService {
    pub async fn new(config: Arc<Config>) -> Result<Self> {
        ...
        let (device_type, session) = Self::initialize_with_acceleration(&config)?;

        info!("✓ Detection service initialized on {}", device_type);
...
// Trait implementation for BubbleDetector
#[async_trait]
impl BubbleDetector for DetectionService {
    async fn detect_bubbles(
        &self,
        img: &DynamicImage,
        page_index: usize,
    ) -> Result<Vec<BubbleDetection>> {
        self.detect_bubbles(img, page_index).await
    }
